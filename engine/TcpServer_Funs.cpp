...
namespace net
{
	...
	void TcpServer::checkConnect(S_CLIENT_BASE* c)
	{
		...
		if(c->closeState == func::S_CLOSE_SHUTDOWN)//已经关闭的话
		{
			...
			else if(temp > 5)//如果还有数据没有接收或者发送完毕的话 超时
			{
		...
		if(c->state == func::S_Connect )
		{
			if(temp > 10)//如果超过十秒还处于连接状态的话
			{
				if(this->onTimeOutEvent != nullptr ) this->onTimeOutEvent(this, c, 2002);//派发超时事件出去
	...
	S_CLIENT_BASE* TcpServer::client(SOCKET socketfd, bool issecurity)
	{
		if(socketfd < 0 || socketfd >= MAX_USER_SOCKETFD) return nullptr;
		auto cindex = LinkerIndexs->Value(socketfd);
		if(cindex == nullptr) return nullptr;
		if(cindex->index < 0) return nullptr;
		auto c = Linkers->Value(cindex->index);
		if(c == nullptr)
		{
			LOG_MSG("client c == null %d-%d line:%d", (int)socketfd, cindex->index, __LINE__);
			return nullptr;
		}
		if(issecurity)
		{
